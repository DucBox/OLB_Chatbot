...
def build_prompt_stage_1(user_input: str, retrieved_chunks: list[dict]) -> str:
    """
    Tạo prompt cho tầng lọc GPT (Stage 1): Nhận vào các chunk JSON + câu hỏi → yêu cầu lọc các chunk liên quan.

    Args:
        user_input (str): Câu hỏi hoặc truy vấn của người dùng.
        retrieved_chunks (list[dict]): Danh sách các chunk có chứa Category, Document Title, Chunk_ID, Content.

    Returns:
        str: Prompt đầy đủ để gửi qua GPT.
    """
...
⚠️ DO NOT include the content of the chunks in the output.
⚠️ DO NOT fabricate any information. Use only the values exactly as they appear.
⚠️ The response must be a pure JSON list without any extra explanation, symbol, or text.
...
def build_prompt_stage_2(user_input: str, final_chunks: list[dict]) -> str:
    """
    Tạo prompt cho Stage 2: sử dụng các chunk đã được lọc từ GPT Stage 1 để sinh câu trả lời hoàn chỉnh.

    Args:
        user_input (str): Câu hỏi hoặc yêu cầu từ người dùng.
        final_chunks (list[dict]): Danh sách các chunk đã được lọc, có Category, Document Title, Chunk_ID, Content.

    Returns:
        str: Prompt hoàn chỉnh để gửi GPT.
    """
