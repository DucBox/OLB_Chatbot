...
def store_embedding_to_db(embedding: list[float], text: str, chunk_id: str, metadata: dict):
    """
    Stores a single embedding with metadata to ChromaDB.

    Args:
        embedding (list): Embedding vector (1536-d).
        text (str): Original chunk content.
        chunk_id (str): Unique ID for this chunk (e.g., doc_id_chunk_0).
        metadata (dict): Metadata for this chunk (doc_id, user, position, etc.)
    """
    try:
        collection.add(
            documents=[text],
        ...
        print(f"✅ Stored: {chunk_id} | Metadata: {metadata}")
    except Exception as e:
        print(f"⚠️ Failed to store {chunk_id}: {str(e)}")
